...
SyncBatchResult SyncProcessor::processJson(const QJsonArray& ops)
{
    ...
    const auto open = sessions.findOpen();
    if (!open.has_value()) {
        result.error = QStringLiteral("no open cash session");
        result.batchValid = false;
        return result;
    }
    ...
    for (const QJsonValue& value : ops) {
        ...
        if (!error.isEmpty()) {
            result.errors.append({ applied.opId == 0 ? op.value(QStringLiteral("opId")).toInt() : applied.opId,
                                   error });
        } else {
            result.applied.append(applied);
        }
...
SyncAppliedOp SyncProcessor::applySale(const QJsonObject& json, int cashSessionId, QString* error)
{
    ...
    const app::data::SaleRecordResult result = m_sales.recordSale(items, cashSessionId, deviceId, /*allowOversold=*/false);
    if (!result.ok) {
        *error = result.error;
        return applied;
    }
...
SyncAppliedOp SyncProcessor::applyDebt(const QJsonObject& json, QString* error)
{
    ...
    const app::data::SaleRecordResult result =
        m_sales.recordCustomerDebt(customerId, items, deviceId, /*allowOversold=*/false);
    if (!result.ok) {
        *error = result.error;
        return applied;
    }
...
SyncAppliedOp SyncProcessor::applyPayment(const QJsonObject& json, int cashSessionId, QString* error)
{
    ...
    const app::data::PaymentResult result =
        m_payments.recordCustomerPayment(customerId, amountCents, cashSessionId, note);
    if (!result.ok) {
        *error = result.error;
        return applied;
    }
    applied.entityId = result.paymentId;
    applied.totalCents = amountCents;
    return applied;
}
